...
const TOP_COMPANIES = [
    ...
    { id: "infosys", name: "🏢 Infosys", roles: ["Developer", "SAP Consultant", "DevOps"] },
...
function getSafeResumePreviewUrl(fileUrl: string, fileType: string) {
    try {
        const parsed = new URL(fileUrl);
        const hostname = parsed.hostname.toLowerCase();
        const isCloudinary = hostname === "res.cloudinary.com" || hostname.endsWith(".cloudinary.com");

        if (parsed.protocol !== "https:" || !isCloudinary) return null;

        if (fileType === "pdf") return parsed.toString();

        // Use Google's viewer for DOCX so the browser doesn't auto-download it.
        return `https://docs.google.com/gview?embedded=1&url=${encodeURIComponent(parsed.toString())}`;
    } catch {
        return null;
    }
}
...
function getSeverityBg(severity: string) {
    switch (severity) {
        ...
        case "major": return "bg-yellow-500/10 border-yellow-500/20";
...
function getImportanceBadge(importance: string) {
    switch (importance) {
        ...
        case "good-to-have": return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/30 text-[10px]">Good-to-Have</Badge>;
        case "bonus": return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/30 text-[10px]">Bonus</Badge>;
        default: return null;
    }
}
...
export default function ResumeDashboard() {
    ...
    const fetchHistory = useCallback(async () => {
        try {
            const res = await fetch(`/api/resume/history?professionalId=${professionalId}`, { credentials: "include" });
    ...
    }, [uploadedResume, professionalId]);

    const fetchCompanies = useCallback(async () => {
        try {
            const opts: CompanyOption[] = TOP_COMPANIES.flatMap((c) =>
                c.roles.map((role) => ({
                    company_id: c.id,
                    company_name: c.name,
                    role,
    ...
    const loadComparison = async (resumeId: number) => {
        try {
            const res = await fetch(`/api/resume/compare?resumeId=${resumeId}&professionalId=${professionalId ?? ""}`, { credentials: "include" });
            const data = await res.json();
            if (data.success) {
    ...
    return (
        <div className="h-full overflow-y-auto p-4 md:p-8 custom-scrollbar">
            <div className="space-y-6 max-w-7xl mx-auto">
                {/* Header */}
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gradient-to-r from-yellow-500/10 to-teal-500/10 p-6 rounded-2xl border border-white/5 backdrop-blur-md">
                    <div>
                    ...
                    {resumes.length > 0 && (
                        <Badge variant="outline" className="border-white/10 text-white/60 px-4 py-1.5 text-sm font-semibold rounded-full bg-white/5">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="grid grid-cols-4 w-full max-w-2xl bg-zinc-900/60 border border-white/5 p-1 rounded-xl">
                    ...
                    <TabsContent value="upload" className="mt-6">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {/* Upload Area */}
                            <Card className="border-white/5 bg-zinc-900/40 backdrop-blur-md hover:border-yellow-500/50 transition-colors">
                                <CardHeader className="pb-3 border-b border-white/5">
                                    <CardTitle className="text-lg flex items-center gap-2 text-white">
                                        <Upload className="w-5 h-5 text-yellow-500" />
                                        Upload Resume
                                    </CardTitle>
                                    <CardDescription className="text-gray-400">PDF or DOCX, max 5MB</CardDescription>
                                </CardHeader>
                      ...
                      ${dragActive
                                                ? "bg-yellow-500/10 border-2 border-yellow-500 border-dashed scale-[1.02]"
                                                : "bg-white/5 hover:bg-white/10 border-2 border-dashed border-white/10 hover:border-white/20"
                                            }
                    `}
                                    >
                                        <input
                                        ...
                                        {uploading ? (
                                            <div className="text-center">
                                                ...
                                                <p className="text-sm text-gray-400">Uploading & parsing resume...</p>
                                            </div>
                                        ) : (
                                            <>
                                                ...
                                                <Badge variant="secondary" className="text-[10px] bg-white/10 text-white hover:bg-white/20 border-transparent">
                                 ...
                                 flex items-center gap-3 backdrop-blur-md"
                                        >
                                            ...
                                            <Badge variant="outline" className="text-[10px] shrink-0 border-yellow-500/30 text-yellow-400 bg-yellow-500/10">
                            ...
                            <Card className="border-white/5 bg-zinc-900/40 backdrop-blur-md relative overflow-hidden group">
                                ...
                                <CardHeader className="pb-3 border-b border-white/5 relative z-10">
                                    ...
                                    <CardDescription className="text-gray-400">Choose company and role to evaluate against</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-5 pt-6 relative z-10">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        >
                                            <SelectTrigger className="w-full bg-white/5 border-white/10 text-white focus:ring-yellow-500">
                                                <SelectValue placeholder="Select a company" />
                                            </SelectTrigger>
                                            <SelectContent className="bg-zinc-900 border-white/10 text-white">
                                                {[...new Map(companies.map((c) => [c.company_id, c])).values()].map((c) => (
                                                    <SelectItem key={c.company_id} value={c.company_id} className="focus:bg-white/10 focus:text-white">
                                                        {c.company_name}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        >
                                            ...
                                            <SelectContent className="bg-zinc-900 border-white/10 text-white">
                                                ...
                                                {["SDE", "Analyst", "Core", "Research", "Manager"].filter(r => !rolesForCompany.includes(r)).map((role) => (
                                                    <SelectItem key={role} value={role} className="focus:bg-white/10 focus:text-white">
                                                        {role}
                                                    </SelectItem>
                                                ))}
                                    ...
                                    {/* Analyze Button */}
                                    <Button
                                        className="w-full h-12 text-base font-bold mt-4 bg-gradient-to-r from-yellow-600 to-yellow-500 hover:from-yellow-500 hover:to-yellow-400 text-white shadow-lg shadow-yellow-500/20 border-0"
                                            ...
                                            <CardContent className="space-y-4">
                                                {currentAnalysis.feedback_json.bulletSuggestions.map((bs: any, i: number) => (
                                                    <div key={i} className="rounded-lg border border-border/50 overflow-hidden">
                                                        ...
                                                        <div className="p-3 bg-yellow-500/5 border-b border-border/50">
                                                            <div className="flex items-center gap-2 mb-1">
                                                                <CheckCircle2 className="w-3.5 h-3.5 text-yellow-500" />
                                                                <span className="text-[10px] font-medium text-yellow-400">AFTER</span>
                                                            </div>
                                        ...
                                        <Card className="hover:shadow-md transition-shadow">
                                           ...
                                           hover:bg-muted/40 transition-colors cursor-pointer"
                                                ...
                                                <div className="flex items-center gap-2 mt-3">
                                                    ...
                                                    {(() => {
                                                        const previewUrl = getSafeResumePreviewUrl(resume.file_url, resume.file_type);
                                                        if (!previewUrl) {
                                                            return (
