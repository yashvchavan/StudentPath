...
const SOURCE_META: Record<SkillSource, { label: string; emoji: string; color: string; bg: string }> = {
  github:   { label: "GitHub",   emoji: "🐙", color: "text-yellow-500", bg: "bg-yellow-500/10 border-yellow-500/30" },
  leetcode: { label: "LeetCode", emoji: "💻", color: "text-orange-500", bg: "bg-orange-500/10 border-orange-500/30" },
  resume:   { label: "Resume",   emoji: "📄", color: "text-amber-500",  bg: "bg-amber-500/10 border-amber-500/30" },
}
...
function ConfidenceRing({ value }: { value: number }) {
  ...
  const color = pct >= 90 ? "#f59e0b" : pct >= 75 ? "#f97316" : pct >= 60 ? "#fbbf24" : "#ef4444"
...
function SkillCard({ skill }: { skill: MergedSkill }) {
  const isVerified = skill.sources.includes("github") || skill.sources.includes("leetcode")
    ...
    <div className="flex items-center gap-3 p-3 rounded-xl border border-border/60 bg-card hover:bg-accent/30 hover:border-border transition-all duration-200 group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          {isVerified && (
            <Badge className="h-5 px-1.5 bg-yellow-500/10 text-yellow-600 border-yellow-500/20 text-[10px] font-semibold">
              Verified
            </Badge>
          )}
        </div>
...
function CourseCard({ course }: { course: CourseResult }) {
  return (
    <a
      ...
      className="group flex gap-3 p-3 rounded-xl border border-border/60 bg-card hover:bg-accent/30 hover:border-yellow-500/40 transition-all duration-200"
    >
      {/* Thumbnail */}
      {course.thumbnail ? (
        <img
      ...
      {/* Info */}
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground line-clamp-2 group-hover:text-yellow-500 transition-colors leading-snug mb-1">
          {course.title}
        </p>
      ...
      <ExternalLink className="w-4 h-4 shrink-0 text-muted-foreground group-hover:text-yellow-500 transition-colors mt-0.5" />
    </a>
  )
}
...
function EmptyState({ onAnalyze, loading }: { onAnalyze: () => void; loading: boolean }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h2 className="text-2xl font-bold mb-2 text-white">Your Skill Passport is empty</h2>
...
export default function SkillPassportPage() {
  ...
  const loadSkills = useCallback(async () => {
    ...
    try {
      const res  = await fetch(`/api/professionals/skills`)
  ...
  useEffect(() => { if (!authLoading && professionalId) loadSkills() }, [authLoading, professionalId, loadSkills])
  ...
  const runAnalysis = async () => {
    ...
    // In Professional Dashboard, the skill analysis actually happens implicitly 
    // when setting LeetCode/GitHub URLs or it can be manually re-triggered
    try {
      const res = await fetch(`/api/professionals/profile/update`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ professionalId, updates: { skills: [] } }) // sending empty array to trigger analysis
      })
      const data = await res.json()
      if (!data.success) throw new Error(data.error ?? "Analysis failed")
      
      // Reload from GET endpoint
      await loadSkills()
    } catch (err: any) {
  ...
  const loadCourses = async () => {
    if (!professionalId || !mergeResult) return
    setCourseLoading(true)
    setError(null)

    const gapSkills = [...mergeResult.skills]
      .sort((a, b) => a.proficiency - b.proficiency)
      .slice(0, 5)
      .map(s => s.skill)

    if (gapSkills.length === 0) gapSkills.push("Communication", "System Design")

    try {
      const res  = await fetch(`/api/recommendations/courses`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ missingSkills: gapSkills, professionalId })
      })
      const data = await res.json()
      if (!data.success) throw new Error(data.error ?? "Course fetch failed")
      setCourses(data.data?.skills ?? [])
      setMissingSkills(gapSkills)
    ...
    <>
      <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <Button
              ...
              className="gap-1.5 border-yellow-500/20 bg-yellow-500/5 text-yellow-500 hover:bg-yellow-500/10 hover:text-yellow-400"
            >
              {courseLoading
            ...
            <Button
              ...
              className="gap-1.5 bg-gradient-to-r from-yellow-600 to-orange-500 hover:from-yellow-500 hover:to-orange-400 text-white"
            >
              {analyzeLoading
                ? <RefreshCw className="w-3.5 h-3.5 animate-spin" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={Award}        label="Total Skills"          value={allSkills.length}      iconClass="text-yellow-500"    iconBg="bg-yellow-500/10" />
          <StatCard icon={Shield}       label="Verified (GitHub/LeetCode)" value={verifiedSkills.length} iconClass="text-orange-500"  iconBg="bg-orange-500/10" />
          <StatCard icon={TrendingUp}   label="Avg Confidence"        value={`${avgConfidence}%`}   iconClass="text-amber-500"   iconBg="bg-amber-500/10" />
          <StatCard icon={AlertTriangle} label="Top Skill Gaps"       value={gapSkills.length}      iconClass="text-yellow-600" iconBg="bg-yellow-500/10" />
        </div>
            ...
            <div className="flex gap-1 p-1 bg-muted/50 rounded-xl w-fit flex-wrap">
              {tabs.map((t) => (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 border border-yellow-500/15
                    ${activeTab === t.key
                ...
                >
                  ...
                  {t.count > 0 && (
                    <span className={`text-xs px-1.5 py-0.5 rounded-full
                      ${activeTab === t.key ? "bg-yellow-500/10 text-yellow-500" : "bg-muted text-muted-foreground"}`}>
                      {t.count}
                    </span>
                  )}
                </button>
                  ...
                  <Card>
                    <CardHeader className="pb-3">
                      <CardTitle className="text-base flex items-center gap-2">
                        ...
                        Technical Skills
                        <Badge className="bg-yellow-500/10 text-yellow-600 border-yellow-500/30 text-xs">{techSkills.length} skills</Badge>
                      </CardTitle>
                  ...
                  <Card>
                    <CardHeader className="pb-3">
                      <CardTitle className="text-base flex items-center gap-2">
                        ...
                        Soft &amp; Non-Technical Skills
                        <Badge className="bg-orange-500/10 text-orange-500 border-orange-500/30 text-xs">{softSkills.length} skills</Badge>
                      </CardTitle>
                        ...
                        {gapSkills.map((s, idx) => (
                          <div key={s.skill} className="flex items-center gap-3 p-3 rounded-xl border border-yellow-500/20 bg-yellow-500/5">
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className="shrink-0 text-xs gap-1 border-yellow-500/20 bg-yellow-500/5 text-yellow-600 hover:bg-yellow-500/10 hover:text-yellow-500"
                              onClick={() => {
