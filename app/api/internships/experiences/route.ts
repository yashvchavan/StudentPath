...
// GET /api/internships/experiences — list all peer experiences (college-scoped)
export async function GET(req: NextRequest) {
    try {
        ...
        try {
            ...
            // Get college_id for scoping experiences to same college
            const [stuRows]: any = await connection.execute(
                "SELECT college_id FROM Students WHERE student_id = ? LIMIT 1",
                [studentId]
            );
            const collegeId = stuRows[0]?.college_id;
        ...
        console.error("[GET /api/internships/experiences]", error);
        return NextResponse.json({ error: "Failed to fetch experiences" }, { status: 500 });
    }
}

// POST /api/internships/experiences — share experience (not tied to a specific posted internship)
export async function POST(req: NextRequest) {
    try {
        ...
        console.error("[POST /api/internships/experiences]", error);
